...
contract AvaxFaucet is AccessControl, ReentrancyGuard {
    ...
    function Pay(address payable receiver) external payable nonReentrant {
        require(GetBalance(payable(_msgSender())) == 0, "AvaxFaucet: Caller must have no AVAX");
        require(GetBalance(payable(AvaxFaucets)) >= Amount, "AvaxFaucet: Contract Empty");
    ...
    function Depo() external payable onlyRole(MANAGER_ROLE) {
        ...
        (bool sent, bytes memory data) = payable(AvaxFaucets).call{value: msg.value}("");
    }

    function UpdateAmount(uint256 newamount) external payable onlyRole(MANAGER_ROLE) {
        require(newamount > 0, "AvaxFaucet: Invalid Amount");
        Amount = newamount;
    }
    ...
    function StuckTokens(address token) external payable onlyRole(MANAGER_ROLE) {
        ...
        _stucktoken.safeTransfer(_msgSender(), _stucktoken.balanceOf(AvaxFaucets));
    }
}