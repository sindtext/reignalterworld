...
contract RAWSSupply is ERC20, AccessControl, ERC20Permit, ReentrancyGuard {
    ...
    function allocate(string calldata utility, address entitled, uint256 amount, uint unlock, uint cliff, uint monthly) external onlyRole(DEFAULT_ADMIN_ROLE) {
        allocation[utility] = Allocation (
            utility, entitled, amount, amount, unlock, 0, monthly, block.timestamp + (cliff * 60 * 60 * 24 * 30)
        );
    ...
    function takeRight(string calldata utility) external onlyRole(MINTER_ROLE) payable nonReentrant {
    ...
    function utilityAllotment(string calldata utility) public view returns (uint) {
