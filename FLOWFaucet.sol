...
contract FLOWFaucet is AccessControl, ReentrancyGuard {
    ...
    function Pay(address payable receiver) external payable nonReentrant {
        require(GetBalance(payable(_msgSender())) == 0, "FLOWFaucet: Caller must have no FLOW");
        require(GetBalance(payable(FLOWFaucets)) >= Amount, "FLOWFaucet: Contract Empty");
    ...
    function Depo() external payable onlyRole(MANAGER_ROLE) {
        ...
        (bool sent, bytes memory data) = payable(FLOWFaucets).call{value: msg.value}("");
    }

    function UpdateAmount(uint256 newamount) external payable onlyRole(MANAGER_ROLE) {
        require(newamount > 0, "FLOWFaucet: Invalid Amount");
        Amount = newamount;
    }
    ...
    function StuckTokens(address token) external payable onlyRole(MANAGER_ROLE) {
        ...
        _stucktoken.safeTransfer(_msgSender(), _stucktoken.balanceOf(FLOWFaucets));
    }
}