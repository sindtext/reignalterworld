...
contract TaikoFaucet is AccessControl, ReentrancyGuard {
    ...
    function Pay(address payable receiver) external payable nonReentrant {
        require(GetBalance(payable(_msgSender())) == 0, "TaikoFaucet: Caller must have no ETH");
        require(GetBalance(payable(TaikoFaucets)) >= Amount, "TaikoFaucet: Contract Empty");
    ...
    function Depo() external payable onlyRole(MANAGER_ROLE) {
        ...
        (bool sent, bytes memory data) = payable(TaikoFaucets).call{value: msg.value}("");
    }

    function UpdateAmount(uint256 newamount) external payable onlyRole(MANAGER_ROLE) {
        require(newamount > 0, "TaikoFaucet: Invalid Amount");
        Amount = newamount;
    }
    ...
    function StuckTokens(address token) external payable onlyRole(MANAGER_ROLE) {
        ...
        _stucktoken.safeTransfer(_msgSender(), _stucktoken.balanceOf(TaikoFaucets));
    }
}