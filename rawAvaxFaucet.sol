...
contract rawAvaxFaucet is AccessControl, ReentrancyGuard {
    using SafeERC20 for IERC20;
    address rawAvaxFaucets;
    
    bytes32 internal constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    bytes32 internal constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    
    iAVAXFaucet iAvaxFaucet;

    constructor() payable {
        _grantRole(MANAGER_ROLE, msg.sender);
        rawAvaxFaucets = address(this);
    }

    function AvaxAdd(address faucet) external payable onlyRole(ADMIN_ROLE) {
        iAvaxFaucet = iAVAXFaucet(faucet);
    }
    
    function AvaxPay(address payable receiver) external payable onlyRole(ADMIN_ROLE) {
        iAvaxFaucet.Pay(receiver);
    }
    ...
    function StuckTokens(address token) external payable onlyRole(MANAGER_ROLE) {
        ...
        _stucktoken.safeTransfer(_msgSender(), _stucktoken.balanceOf(rawAvaxFaucets));
    }
}

interface iAVAXFaucet{
